...
\centering
\figfont{A}\hfill\\
\resizebox{3.5in}{!}{\includegraphics{SimpleCircuit3}}
\figfont{B}\hfill\\
\resizebox{3.5in}{!}{\includegraphics{NetworkProjections3}}
...
parameters shown in Table~\ref{tab:GA:Genome}.  The GA was run with each cost
function using two conditions: 1) with identical ANF spike times as
used in creating the target data, and 2) with different ANF spike
times, derived from the same instantaneous rate function but where the
spike times were recalculated for each evaluation.  The performance of
the GA was evaluated by examining the behavior of the best genomes in
relation to the scores of other genomes with small parameter
deviations, the relative parameter difference between the best genome,
and target genome (parameters of the target network) and the
robustness of the optimization when using different ANF inputs.
...
\multicolumn{2}{l}{Synapse Parameters}       & \\ %\hline
     $\tau _{{\rm AMPA}}$       &         0.36~ms         & ventral CN neurons, mature guinea   pig \cite{GardnerTrussellEtAl:1999} \\ %\hline
