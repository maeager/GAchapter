...
inputs to the trial network are identical to those used in the 25 repetitions
...
(100 and 25 respectively). The maximum \PsiIFR value observed in this study was
...
range of frequencies in the normal hearing cat model \citep[see review][]{Lopez-Poveda:2005}. The auditory filterbank
...
\IFR response that was down-sampled to 10 kHz. Twenty \HSR and ten
\LSR\space \AN fibres were simulated for each frequency-channel. Spike times were generated
independently for each fibre from the \IFR using a
...
current-based synapses were used to model the \CNSM, as described in Chapter \ref{sec:MethodsChapter}.  Type I neurons
...
inputs were randomised and the training data (25 repetitions) remained the same, the
...
weighting values rather than traditional gradient descent techniques such as back
propagation, lies in the inherent problems associated with gradient descent
...
# a real network, but
The development of methods for constraining new \BNN models is an important step for
