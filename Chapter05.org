...
model construction. Bottom-up approaches to neural modelling have proven
successful for representing single cell models, generally relying on hand-tuning
parameters. They are, however, insufficiently sophisticated when it comes to
constraining the free parameters in small networks of neurons
\citep{GrillnerMarkramEtAl:2005,KochSegev:1998}. In contrast, top-down
approaches to constraining neural network models uses the output of the network
...
relationships of \HH-type models are not mathematically suitable for error
back-propagation.
...
\citep{KostalLanskyEtAl:2007,BrownKassEtAl:2004}. Inferring the connectivity
within a network requires a cost analysis or metric of the spiking output
\citep{Victor:2005,BretteRudolphEtAl:2007}. Developments in this area have
progressed from ensemble feature-based methods
...
based on measures of (1) spike-timing, (2) \IFR, or (3) \AIV. Surrogate data
from a randomly generated network model served as our target and the ability of
the algorithms to find the known neural parameters was assessed. To analyse the
...
The exemplar network used in this chapter is a simplified version of the \CNSM
model.
...
of the \CN, to organised isofrequency lamina. \ANFs are
...
#+Attr_LaTeX: width=0.45\textwidth
#+CAPTION: [Network projections]{ANFs are ordered into a wide range of frequency channels that are mapped to the VCN and DCN in a tonotopic fashion. Topographic organisation of lateral connections in the CN stellate network shows the range of inputs to TS cells from Golgi, DS and TV cells. Dendritic cell morphologies characterise the range of ANF inputs and hence determine their frequency response. ANF inputs to TS and TV cells are restricted to one isofrequency lamina, whereas DS dendrites span one third of the VCN\@. Axonal plexi of DS cells typically cover one third of the CN and one half of the DCN, giving them a strong influence throughout the CN \citep{ArnottWallaceEtAl:2004}.}
...
in the \VCN. The dendrites of \DS cells cover one third of the cross-frequency
axis in the \CN, contributing to this cell's wide frequency response. In turn,
this cell is responsible for altering the frequency responses in \TS and \TV
cells \citep{SpirouDavisEtAl:1999}. \DS cells are coincidence detectors and have
a precisely timed onset response that affects the temporal properties of \TS
cells \citep{PaoliniClareyEtAl:2005,RhodeGreenberg:1994a} and completely inhibit
\TV cell responses to loud clicks \citep{SpirouDavisEtAl:1999}. GABAergic
inhibition from Golgi cells (Figure \ref{fig:GA:CNdiagram}) modulates the level
of excitation necessary to reach threshold for all \CN cells
...
the population. In this process, a model is represented by a genome, which is
the result of mapping the model parameters into binary strings and concatenating
...
is negative here, instead of the conventional positive, because we used cost
functions corresponding to an error term, so that smaller values of $c_{i}$ imply
greater fitness). Following selection of a genome, crossover occurred with a
strictly different selected genome, with probability 0.95. Alternatively, the
...
models the strength of synapses from \ANFs to \TS cells. It was encoded over the
...
concatenating the parameter bit strings in the order given in Table
\ref{tab:GA:Genome}.
...
#+BEGIN_LaTeX
  ...
  \footnotesize{Units of weights are \uS. $n$ and $s$
    ...
    parameters was set to 0.0001 \uS and other parameters to 1.}
  \end{table}
#+END_LaTeX
...
shows a spike raster plot for the response of all \TS cells to the notch noise
...
\ref{fig:GA:Costfunctions}B shows the responses to 100 repetitions for a single
...
#+BEGIN_LaTeX
  ...
  \caption[Cost functions]{Cost function measures derived from the
    ...
    network (CF 3.45kHz) from 100 repetitions of the stimulus are
    shown. This cell was placed at the edge of the spectral notch (arrow
    in (A)). (C) PSTH response of the same \TS cell used in (B) (bin
    width 0.25~msec, 100 repetitions). Note the regularly-spaced peaks at the
    ...
    voltage, smoothed from 100 repetitions, for the same \TS cell. There
...
inputs to the trial network are identical to those used in the 25 repetitions of
the target data and the target network parameters are used, the value of \PsiST
is zero. The maximum value of \PsiST observed in this study was approximately
360 msec. For an example trial network that produces the correct number of
spikes for each neuron but with an average spike timing error of 1 msec, given
that the average number of spikes per train is 9, the cost function would be
$\PsiST=9$ msec per spike train.
...
time bins ($<$ 1 msec), the \PSTH is an estimate of the \IFR. The \IFR
...
and /W/ is the bin width of the \PSTH. The unit for \PsiIFR is spikes per
...
variability, target data from 100 repetitions was used to generate a higher
...
example of a \TS cell's \PSTH produced from 100 repetitions of a notch noise
...
the numbers of repetitions used to create $r_{i}^\ast$ and $r_{i}$ are different (100
and 25 respectively). The maximum \PsiIFR value observed in this study was
approximately 0.5 spikes/msec per stimulus per neuron. For a trial network, if
the average \PSTH error is 10 spikes over all bins, then \PsiIFR is
approximately 0.2 spikes/msec.

*** Average Intracellular Voltage (AIV) Cost Function

\Gls{AIV} responses reflect the influence of excitatory and inhibitory inputs on
a neuron. This may be a more reliable way of determining the strength of
synaptic inputs, since spike times and \PSTHs do not convey any information
about the subthreshold activity of a neuron. The intracellular membrane
potential waveform has been used to constrain single neural models with
deterministic current inputs and no synaptic noise
\citep{KerenPeledEtAl:2005,VanierBower:1999}. Recordings in real neurons are
more varied, with background noise, thermal noise in synapse and membrane
channels and non-linear effects of \APs. \AIV alleviates this noise by averaging
membrane potential recordings over multiple repetitions. Previous studies in
the \CN have used experimental \AIV to categorise physiological responses,
...
examples of \AIV waveforms, $\bar{v}$, from a \TS cell averaged over 25 and 100
...
of repetitions used to create $\bar{v}_{i}$ and $\bar{v}_{i}^{\ast}$ (25 and 100,
...
kHz. A 50 msec stimulus was presented at 60 dB \SPL with 5 msec onset/offset
ramps, a 20 msec delay and 10 msec pause after the stimulus. Notch noise stimuli
...
#+BEGIN_LaTeX
  \begin{figure}[th]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{../GAChapter/gfx/notchstim.png}
    \caption[Notch stimulus and HSR ANF filterbank output]{Notch stimulus and HSR ANF filterbank output. The stimulus is in Pascals (dB SPL is defined as 20 $\mu$Pa).}
    \label{fig:NotchStim}
  \end{figure}
#+END_LaTeX
...
range of frequencies in the normal hearing cat model \citep[see
review][]{Lopez-Poveda:2005}. The auditory filterbank used in this study
consisted of sixty frequency channels with centre frequencies between 0.2 and 30
kHz, with other simulation parameters as listed in Table
...
The level of spontaneous activity in \HSR and \LSR\space \AN fibres was set to
50 and 0.5 Hz, respectively. The stimulus was passed through the \AN model for
each frequency channel for both \LSR and \HSR fibres, producing an \IFR response
that was down-sampled to 10 kHz. Twenty \HSR and ten \LSR\space \AN fibres were
simulated for each frequency-channel. Spike times were generated independently
for each fibre from the \IFR using a pseudo-random spike generator
\citep{JacksonCarney:2005}, with refractory effects similar to those present in
\ANFs.
...
current-based synapses were used to model the \CNSM, as described in Chapter
\ref{sec:MethodsChapter}. Type I neurons respond to current injection with
  ...
  \begin{table}[tp]
    ...
    \begin{tabularx}{0.8\linewidth}{lcccc}\toprule
      ...
      Current Clamp Model     &  I-t   &  I-II  &   I-c   & I-c \\[0.5ex] \midrule
       \gNa, S/cm$^{2}$       & 0.235  & 0.235  &  0.235  & 0.235 \\
       \gKHT, S/cm$^{2}$      & 0.018  &  0.02  &  0.019  & 0.019 \\
       \gKLT, S/cm$^{2}$      &   0    & 0.0047 &    0    & 0 \\
       \gKA, S/cm$^{2}$       & 0.0153 &   0    &    0    & 0 \\
       \gh, mS/cm$^{2}$       & 0.0618 & 0.247  & 0.06178 & 0.6178 \\
      \gleak, mS/cm$^{2}$     & 0.471  & 0.471  &  0.471  & 0.962 \\
...
The connectivity and network parameters were described in detail in Section
\ref{sec:Methods:ConnectivityandTopology}. The synapse models and their delay
parameters in this analysis are unchanged from those reported in Section
\ref{sec:Methods:DelayLatency}, in Chapter \ref{sec:MethodsChapter}.
Topographical connectivity in this model was based on position within the \CN
(Figure \ref{fig:GA:MicroCN}B), but is easily interchangeable with
frequency-specific connectivity. Connection parameters that were fixed are shown
in Table \ref{tab:GA:GeneralParams} and parameters used in the optimisation are
shown in Table \ref{tab:GA:Genome}.
...
within isofrequency laminae, so \ANF inputs were chosen from fibres in the same
...
a typical physiological response to frequencies 2 octaves below and 1 octave
...
time step of 0.1 msec. \GAs and sensitivity analyses were implemented in [[latex:progname][C++]]
using [[http://lancet.mit.edu/ga][GAlib]] \citep{Wall:2006} and [[http://www.pvm.com][PVM]] libraries
\citep{GeistBeguelinEtAl:1994}. \GA simulations were distributed on a cluster of
nine PCs (3 GHz Pentium4) and a 64-CPU SGI Altix[fn:: Computer system named
\textsf{soma} at the Department of Electrical and Electronic Engineering and
Neuroimaging Group, University of Melbourne in 2006.] with a master-slave
paradigm.

** Analysis of Genetic Algorithms and Cost Functions

To test the performance of the cost functions in \GA optimisations, sets of
...
\IFR function but where the spike times were recalculated for each
...
* Results: Optimisation of Biophysically-Realistic Neural Networks Using Different Inputs
  :PROPERTIES:
...
This section presents the results and analysis of GA optimisation on network
parameters of the \CN stellate microcircuit model.
...
#+BEGIN_LaTeX
  \begin{figure}[t!]
    \figfont{A}\hspace{2in}\figfont{B}\hspace{1.8in}\figfont{C}\\
    %\centering
    \includegraphics[width=\textwidth]{All25GAPerf-Stretch}
    \caption[Performance of the GA (25 repetitions)]{Performance of the GA's best
...
#+BEGIN_LaTeX
  \begin{figure}[th!]
    ...
    \includegraphics[width=\textwidth]{../GAChapter/src/boxplot25-sep.pdf}
    \caption[Cross comparison of best genomes (25 repetitions)]{Cross comparison of best
      ...
      The box plots show the three best genomes evaluated ten times for
      each cost function, plus an accumulation box plot of all three. 100
...
basis upon which the \GA performs optimisation by comparing candidate genomes to
the target.
...
# \note{but further explanation   is unknown.}
...
#+BEGIN_LaTeX
  ...
  \begin{figure}[t!]
    \figfont{A}\hspace{2in}\figfont{B}\hspace{1.8in}\figfont{C}\\
    \includegraphics[width=\textwidth,keepaspectratio]{../GAChapter/gfx/All100GAPerf-Stretch.pdf}
    \caption[Performance of the GA (100 repetitions)]{Performance of the GAs
      best performing genome run with 100 repetitions in the fitness function. GA
      ...
      standard deviation away from the mean target genome score.}
    \label{fig:GA:R5}
  \end{figure}
...
three different graphs (Figure \ref{fig:GA:R5}A-C) correspond to evaluation of
...
while the top of the (appended) dark bars give the mean score when only 25
...
#+BEGIN_LaTeX
  ...
  \includegraphics[width=\textwidth,keepaspectratio]{../GAChapter/gfx/Histograms100-MaxNorm}
    \caption[Cost function histograms of parameter perturbations (100
      repetitions)]{Cost function histograms of simultaneous parameter perturbation using 100
...
#+BEGIN_LaTeX
  \begin{table}[th]
    ...
    \begin{tabularx}{0.95\textwidth}{Xccc} \toprule
  Cost function  & PE$^\ast$ & Final \GA Score & Mean (S.D)\\[0.5ex]\midrule
     ST (msec)     & 1.977  &    7.86038     & 7.89 (0.04) \\
  IFR (spikes/msec)& 2.169  &    0.154698    & 0.1557 (8.6e-4) \\
   AIV (mV/msec)   & 2.325  &   0.0292369    & 0.0292 (9.8e-5)\\ \bottomrule
  \end{tabularx}
  \end{table}
#+END_LaTeX


# \note{Boxplot difficult to read}
#+ATTR_LATEX: width=\textwidth
#+Caption: [Cross comparison of best genomes (100 repetitions)]{Cross comparison of best genomes generated using GAs with 100 repetitions, measured against the target, 1-step and 5-step parameter perturbation distributions. The box plots show the best genomes evaluated ten times for each cost function.}
#+LABEL: fig:GA:R7
[[../GAChapter/src/boxplot-100+25.pdf]]
...
# \note{This is an attempt to include excess material into the  thesis that was chucked out after the failure of the JNeuroPhysiol  submissions. Anything with 250 repetitions has been removed}

** Genetic Algorithm Performance


# \note{Summary of Ideal Input {GA} performance}
...
generations and the best genome score was 9.72 msec (Figure \ref{fig:GA:5}B).
The best genome for the identical inputs was also closer to the target parameter
...
evolved quite rapidly before settling between 0.3 and 0.25 sp/msec. The
histogram of evaluated scores peaks around 0.25 sp/msec with a tail toward 0.2
sp/msec. The best genome's score of 0.195 sp/msec is equivalent to an average
\PSTH error of 11.8 spikes per cell. In terms of the parameter error from the
target, the \IFR-25 cost function with identical inputs returned the closest
genome to the target of 0.201 (Figure \ref{fig:GA:8}C) for all \GA
simulations. When the inputs were randomised and the training data (25
repetitions) remained the same, the \GA populations' learning was considerably
slower and the search space was more compact, Figure 6B.
# \note{linkback to previous  section}.
...
# \note{remove or replace 250 with 100}
...
# \note{refine J Neurophysiol section to go here}
...
(around 0.15 and below 0.2 spikes per msec), and a majority of 5-step scores
above the distribution of 1-step scores. Different \ANF inputs had an adverse
effect on the learning performance of the \IFR-25 cost function, with the \GA
unable to find reasonable estimates near the global optimum (Figure
\ref{fig:GA:10}B). The 1 step and 5 step scores were distributed around or close
to the target scores showing a compression of the global optimum around 0.25
sp/msec (Figure \ref{fig:GA:10}B). For the \IFR cost function with 250
repetitions, the results are improved with the target shifted lower and the \GA
searching closer to the optimal genome (Figure \ref{fig:GA:10}C). The target and
1 step distribution lie around 0.16 sp/msec and the 5-step scores are spread up
to 0.2 sp/msec.
...
effectiveness of optimisation, especially gradient-descent methods. For
...
#  gradient-descent methods. For parameters with a target value close to
...
# \note{Correlation between relative param error and gradient in Figure \ref{fig:GA:BestGenomeVGradient} to be discussedfurther. The LogLog results indicate a confirmation of our suspicion that the gradient would directly effect the eventual outcome of the best genomes. }
...
#+BEGIN_LaTeX
  ...
  LogLog (Pass Only) & -0.322  & 0.00284  & 0.141 & 9.7 & 0.338 \\[0.5ex] \hline
     ...
     LogLog (All)    & -0.341  & 3.63e-06 & 0.217 & 24.4& 0.363 \\[1ex] \bottomrule
   \end{tabular}
  \end{table}
  \begin{table}[th]
    \centering
  ...
  LogLog (Pass Only) & -0.583 & 0.00177  & 0.205  & 11.1 & 0.324  \\[0.5ex] \hline
     ...
     LogLog (All)    & -0.317 & 1.65e-05 & 0.191  & 20.8 & 0.29   \\[1ex] \bottomrule
  \end{tabular}
  \end{table}
  \begin{table}[th]
    \centering
  ...
  LogLog (Pass Only) & -0.344 &  0.0122  & 0.126  & 11.1 & 0.324\\[0.5ex] \hline
     ...
     LogLog (All)    & -0.368 & 1.81e-07 & 0.267  & 20.8 & 0.29 \\[1ex] \bottomrule
  \end{tabular}
  \end{table}
#+END_LaTeX

# \note{This paragraph has not been vetted.}
...
investigated, based on spike timing, firing rate (\IFR) or intracellular recordings (\AIV). Performance of the \GA with the three different cost
...
relative parameter errors between the best genomes and the target genomes. Our
analyses showed that all three cost functions gave a similar overall level of
...
was good, but not as consistent as the \ST cost function. For both types of
...
the most successful at mimicking the spike timing behaviour of the target network, while \IFR
...
Taken together, these findings indicate that noise was not a significant
...
constraining these type of given data sets with $N$ repetitions, it may be helpful
...
the \GA using two values of repetition numbers (e.g.  $N$ and $N\times100$) and
undertaking a cross-comparison of the kind performed here to judge whether any
improvement in fit can by achieved by increasing the number of repetitions. If an
improvement does occur, this indicates that there may be a benefit in
...
#
#  \note{David: para unclear} Given different characteristics of the cost
...
#  neurons.\note{?} The temporal resolution of the \IFR cost function is
...
#  the level of the ideal scores (Figure\space\ref{fig:11}A,C).\note{unclear}
#
...
#  significantly\note{needs numbers to show this}. In all cost
#  functions, the flattening of the cost function search spaces
#  \note{explain?} around the target parameters contributed to an overlap
...
microcircuits is increasingly gaining the interest of neuroscientists
...
Gradient-descent methods are easily trapped in local minima and have difficulty
...
# \note{References required in this paragraph}
# Evolution of the connection of weights can use \GAs then back propagation for local refining.
Hybrid techniques may be advantageous, considering network structure and
synaptic weights may not be well suited to the same optimisation method
\citep{AngelineSaundersEtAl:1994,SohnDagli:2004}.

Using experimental data from populations of neurons adds more complexity to the
problem of constraining \BNNs. Optimising spiking neural networks to an exact
solution relies on analytically differentiable neural models and synapses
\citep{BretteGuigon:2003,Brette:2006}, and other analytical methods have been
reviewed \citep{Brette:2007,EggertHemmen:2001}. Alternative methods to infer
connectivity and weights on a generic spiking model using extracellular spiking
data have had some success \citep{MakarovPanetsosEtAl:2005}, but the underlying
properties of neuron types and the size of populations are restrictive. Small
\BNNs enable the manipulation of cell-based as well as network-based parameters
...
combination of spike timing, \AIV or internal state information has advantages
over individual methods
\citep{VanierBower:1999,KerenBar-YehudaEtAl:2009,KerenPeledEtAl:2005,RossantGoodmanEtAl:2010}.

Intracellular voltage data has been used in constraining the membrane
conductances of multi-compartmental single-neuron models
\citep{LeMasson:2000,KerenPeledEtAl:2005}. These methods are not always
effective in single simulations unless combined with other cost
functions. Intracellular voltage data has been shown to be very effective in
optimising membrane parameters \citep{VanDeEtAl:2008,KerenPeledEtAl:2005} but
would not be suitable for a \BNN as inter-spike intervals
\citep{KerenPeledEtAl:2005}. Phase-plane analysis due to variation in the
synaptic input and the loss of temporal information. It is not currently
possible to obtain simultaneous intracellular voltage recordings from more than two neurons let
alone a whole nucleus, but limited \AIV data could be used in conjunction with
other cost functions to constrain \BNN models.

#  \GA could not have done an better than any other optimisation techniques.
#  Evolutionary vs Grad descent studies?  Consistency, efficiency for \BNNs.
...
#  establish the weighting values rather than traditional gradient descent
#  techniques such as back propagation, lies in the inherent problems
#  associated with gradient descent techniques. Back-propagation is easily
...
# \note{stochastic versus stereotypical principles of synaptic  organisation and cell types}
...
# \note{Hamish noted that this para was not applicable}
...
information about the sub-threshold behaviour of neurons in the network, which
is not available in the \ST and \IFR cost functions. Intracellular voltage data
...
#
#  \note{*No real differences in eventual performance despite reduction in score \\
...
#  ** noteworthy  \AIV limitations from 25 repetitions were removed  in 100 repetitions, with the best genome's score were closer to the target  (less than mean of 5 unit step perturbation).\\
#  }
#
...
eventual performance. There is no clear advantage of best genomes obtained
using 100 or 25 repetitions, as shown the cross comparison charts in
...
# \note{{GA} could not have done an better than any other
# optimisation techniques. Evolutionary vs Grad descent studies?
# Consistency, efficiency for \BNNs.}

# \note{*Studies with other cost functions - do they get close to
...
back propagation, lies in the inherent problems associated with gradient descent
...
and non-differentiable makes \GA approaches a good chance for success. Using
hybrid techniques overcomes limitations of \GAs in simultaneously finding
network structure and weights for recurrent neural networks
\citep{AngelineSaundersEtAl:1994,SohnDagli:2004}. Using analytical neural
models has seen the best attempt at optimising ensemble of neurons to
experimental data \citep{RossantGoodmanEtAl:2010}.

# \note{Combination techniques:
...
# \note{Sohn and Dagli 2004: {GAs} have been used to select the
...
of 240 \HH-like cells, simulated in [[latex:progname][NEURON]] and took approximately 90 seconds to
run a 80 msec stimulus on a 1.8 GHz CPU (32-bit Itanium, SGI Altix).
Evaluations the \AIV and \IFR cost functions were a minor fraction of the total
...
# a real network, but
The development of methods for constraining new \BNN models is an important step
for generating microcircuits and larger networks. The study reported in this
...
# \note{Last section you need to improve  when you are done}
