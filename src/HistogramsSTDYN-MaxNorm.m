...
load ../../../octave/ReRun13-01/rand1norm
load rand1diffAN
load ../../../octave/ReRun13-01/rand5norm
load rand5diffAN
...
xst = linspace(0,50,1000);



[ganormhist xst] = hist(stdyn_normal(:,cf_pos),xst);%stdynhist_s = filter(hanning(filter_width),1,stnormhist);
[gadiffhist xst] = hist(stdyn_diffAN_sim1(:,cf_pos),xst);%ivhist_s = filter(hanning(filter_width),1,stdiffhist);
...
outfile = "HistogramSTDYN-MaxNorm.dat";
fd = fopen(outfile, "wt");
for i=1:length(xst)
...
% ylabel('count','FontSize',14)
% %print -depsc2 -r600 c:/cygwin/home/eagerm/thesis/thesis/newgfx/STDYNAreaHistogram_v4.eps
