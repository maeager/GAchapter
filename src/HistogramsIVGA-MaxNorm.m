...
[ganormhist xst] = hist(ivga25_normal(:,cf_pos),xst); %ivga25hist_s = filter(hanning(filter_width),1,stnormhist);
[gadiffhist xst] = hist(ivga25_diffAN_sim2(:,cf_pos),xst); %ivhist_s = filter(hanning(filter_width),1,stdiffhist);
%[gadiffhist1 xst] = hist(ivga25_diffAN(:,cf_pos),xst);%ivhist_s =
				%filter(hanning(filter_width),1,stdiffhist);
%[gadiffhist2 xst] = hist(ivga25_diffAN(:,cf_pos),xst);%ivhist_s =
				%filter(hanning(filter_width),1,stdiffhist);
%[gadiffhist3 xst] = hist(ivga25_diffAN(:,cf_pos),xst);%ivhist_s = filter(hanning(filter_width),1,stdiffhist);
...
outfile = "HistogramsIVGA-MaxNorm.dat";
fd = fopen(outfile, "wt");
for i=1:length(xst)
...
% ylabel('count','FontSize',14)
% %print -depsc2 -r600 c:/cygwin/home/eagerm/thesis/thesis/newgfx/IVGA25AreaHistogram_v4.eps
